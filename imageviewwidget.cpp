...
ImageViewWidget::ImageViewWidget(QAbstractScrollArea* parentScrollArea, QWidget *parent)
    : QWidget(parent)
{
...
double ImageViewWidget::getZoomFactor()
{
    return zoomFactor;
}

void ImageViewWidget::setZoomFactor( double f )
{
    int w, h;
...
void ImageViewWidget::setPixmap(const QPixmap& pixmap)
{
    ...
    setZoomFactor(min(width() / (double)m_pm->width(), height() / (double)m_pm->height()));
    setMinimumSize(static_cast<int>(m_pm->width()*zoomFactor), static_cast<int>(m_pm->height()*zoomFactor));
...
void ImageViewWidget::paintEvent( QPaintEvent *event )
{
    ...
    imgVisible.setWidth(m_pm->width());
...
QMatrix ImageViewWidget::getMatrixInv() const
{
    return currentMatrixInv;
}

QMatrix ImageViewWidget::getMatrix() const
{
...
void ImageViewWidget::keyReleaseEvent(QKeyEvent * event)
{
    if (event->key() == Qt::Key_Shift) {
        browsing = false;
        event->accept();
